...
`serializedProgramCompressedBytes` 是 GZip 压缩后的 Udon Program 序列化结果.

= Udon Program 的反序列化
`serializedProgramCompressedBytes` 经过 GZip 解压后得到的二进制文件是 `UdonProgram` 实例序列化后的结果.

这个序列化过程使用的是 #link("https://github.com/TeamSirenix/odin-serializer/")[`OdinSerializer`]. 所以我们可以直接用这个序列化器对应的反序列化器进行反序列化. 一些关键代码如下
