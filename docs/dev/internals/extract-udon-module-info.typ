...
#cross-link-heading("/dev/udon/udon.typ", [== 执行过程])[Udon VM - 执行过程]和 #cross-link-heading("/dev/udon/udon.typ", [== 外部函数])[Udon VM - 外部函数]中解释了 Udon VM 如何调用外部函数. 为了正常识别外部函数调用, 并重建更高级的语义信息, 本反编译器需要知道每个 `externSignature` 所对应函数的参数数量, 原始名字, 具体类型(`method`, `ctor`, `op`, `field`), 是否有返回值, 是否为静态函数等信息.
...
- 对于每个类中的每个函数, 使用其函数签名和 `UdonNodeDefinition` 中的参数列表信息推断我们需要的所有信息, 具体而言
  ...
  - 推断是否为静态函数: 若没有参数则为静态函数; 若有参数, 则检查 `UdonNodeDefinition.parameters.First()` 的 `name` 是否为 `instance` 且类型是否与该类相同.
