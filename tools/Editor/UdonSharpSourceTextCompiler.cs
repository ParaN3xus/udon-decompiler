...
public static class UdonSharpSourceTextCompiler
{
    ...
    public static event Action<string> OnResultCompleted;
    public static event Action<List<string>> OnBatchResultCompleted;
    ...
    private static void ExecuteBatchCompilation()
    {
        ...
        foreach (var programAsset in assetsToCompile)
        {
            ...
            if (program != null)
            {
                encodedResults.Add(EncodeProgramAsCompressedHex(program));
            }
            else
            {
        ...
        if (encodedResults.Count == 1)
        {
            OnResultCompleted?.Invoke(encodedResults[0]);
        }
    }

    private static string EncodeProgramAsCompressedHex(IUdonProgram program)
    {
