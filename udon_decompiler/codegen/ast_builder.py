...
class ASTBuilder:
    def __init__(
        ...
        function_analyzer: FunctionDataFlowAnalyzer
    ):
    ...
    def _add_variable_declarations(self, block: BlockNode) -> None:
        ...
        for var in self.analyzer.variables.values():
            if var.scope in (VariableScope.LOCAL, VariableScope.TEMPORARY, VariableScope.PARAMETER):
    ...
    def _build_block_statements(
        ...
        if structure:
            self._build_control_structure(
                structure, parent_block, structures, visited)
        else:
    ...
    def _build_if_statement(
        ...
        for block in structure.true_branch:
            if block not in visited:
                self._build_block_statements(
                    block, then_block, all_structures, visited)
    ...
    def _build_while_statement(
        ...
        if structure.loop_body:
            for block in structure.loop_body:
                if block not in visited:
                    self._build_block_statements(
                        block, body, all_structures, visited)
    ...
    def _extract_condition_from_block(self, block: BasicBlock) -> Optional[ExpressionNode]:
                ...
                if state and len(state.stack) > 0:
                    ...
                    if cond_value:
                        ...
                        if var:
                            return VariableNode(
                                var_name=var.name,
                                var_type=var.type_hint
                            )

        return VariableNode(var_name="<condition>")

    def _translate_basic_block(self, block: BasicBlock, parent_block: BlockNode) -> None:
        for inst in block.instructions:
    ...
    def _create_assignment_statement(self, expr: Expression, inst: Instruction) -> AssignmentNode:
